...
import static java.util.Arrays.copyOf; //第一次见直接导入静态方法

/**
 * 自定义 ArrayList 集合，省掉了很多的判断检查，参考环境 jdk 1.8
 *
 * @param <T>
 */
public class MyArrayList<T> {
    ...
    public MyArrayList() {
        this.element = EMPTY_ELEMENTDATA;//初始化数组，元素为空
    }
    ...
    private void changeCapacitySize(int minCapacity) {
        ...
        int newCapacity = oldCapacity + (oldCapacity >> 1);//新容器长度为旧容器的 1+1/2 倍（也不知道有啥用）
